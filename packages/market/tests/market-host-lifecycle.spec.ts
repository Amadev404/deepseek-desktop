...
describe('community market Host capability lifecycle', () => {
  const previousUserData = process.env.DEEPSEEK_DESKTOP_USER_DATA

  beforeEach(() => {
    process.env.DEEPSEEK_DESKTOP_USER_DATA = 'C:/fixture-user-data'
  })

  afterEach(() => {
    if (previousUserData === undefined) delete process.env.DEEPSEEK_DESKTOP_USER_DATA
    else process.env.DEEPSEEK_DESKTOP_USER_DATA = previousUserData
  })

  it('publishes browsing, managed package operations, and desktop actions together', async () => {
    ...
    await expect(harness.request(marketRoutes.state)).resolves.toMatchObject({
      status: 200,
      body: { desktopActions: { openTerminal: true, requestRestart: true } },
    })
