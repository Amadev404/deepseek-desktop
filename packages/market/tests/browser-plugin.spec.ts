...
describe('community market browser plugin', () => {
  ...
  it('registers one shared Market surface in official settings without fetching', () => {
    const b = bench()
    for (const name of ['settings.plugins.tab', 'shell.overlay']) b.declare(name)
  ...
  it('follows late declaration and declaration reload for both entries', () => {
    const b = bench()
    b.apply()
    const names = ['settings.plugins.tab', 'shell.overlay'] as const
